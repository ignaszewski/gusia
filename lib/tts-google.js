...
setInterval(function() {
    if(playerQueue.length>0 && !playerPlaying) {
        if(existsFile.sync(playerQueue[0])) {
            playerPlaying = true;
...
const tts = function(text) {
    const chars = {'ą': 'a', 'ć': 'c', 'ę': 'e', 'ł': 'l', 'ó': 'o', 'ń': 'n', 'ś': 's', 'ź': 'z', 'ż': 'z'};
    let filename = text.toLowerCase().substr(0,100).replace(/[ąćęłóńśźż]/gi, match => chars[match]);
    filename = filename.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    const dest = path.resolve(__dirname+'/../mp3/', filename+'.mp3');
    playerQueue.push(dest);
    if(!existsFile.sync(dest)) {
        co(function * () {
