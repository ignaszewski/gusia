...
const stt = function() {
    ...
    rec.start({sampleRate: sampleRate}).pipe(file).on('finish', ()=>googleSTT(dest));
};

const googleSTT = function (fileName) {
    console.log('...Wysylam (google)...');
    request.post({
...
const parseResult = function (err, resp) {
    ...
    let confidence = resp.toJSON().body.results ? ((resp.toJSON().body.results[0].alternatives[0].confidence)*100).toFixed(2) : null;
    if(trans !== null) {
        console.log(trans, confidence+'%');
    }
    stt();
};
